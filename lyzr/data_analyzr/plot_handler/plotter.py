...
from lyzr.data_analyzr.plot_handler.models import (
    PlottingStepsModel,
    PlotingStepsDetails,
)
...
class PlotFactory(FactoryBaseClass):
    ...
    def __init__(
        ...
        **llm_kwargs,  # model_kwargs: dict
        ...
        self._rerun_analysis = set_analysis_attribute(
    ...
    def get_visualisation(self, user_input: str, **kwargs) -> str:
        ...
        if self.steps.df_name not in self.plot_df_dict:
            plot_df = self.plot_df_dict[list(self.plot_df_dict.keys())[0]]
        else:
            plot_df = self.plot_df_dict[self.steps.df_name]
        self.fig = self.make_plot_figure(df=plot_df)
        if kwargs.pop("auto_train", self.params.auto_train) and self.fig is not None:
    ...
    def get_prompt_messages(self, user_input: str) -> list:
        ...
        messages = [
            LyzrPromptFactory(name="plotting_steps", prompt_type="system").get_message(
                ...
                plotting_lib=self._plotting_library,
                schema=schema,
        ...
        question_examples_list = self.vector_store.get_similar_plotting_steps(
            user_input
        )
    ...
    def _plot_subplot(
        ...
        columns = get_columns_names(
            ...
            columns=list(flatten_list([plot.x, plot.y])),
        )
    ...
    def save_plot_image(self) -> str:
        plt.tight_layout()
        if not PlotFactory._savefig(self.fig, self._plot_path):
            self.logger.error(
                f"Error saving plot at: {self._plot_path}. Plot not saved. Displaying plot instead. Access the plot using `.fig` attribute.",
                extra={
